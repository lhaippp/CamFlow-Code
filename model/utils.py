...
def WarpMat(offset, ori_size, patch_size):
    ...
    src_pt = torch.Tensor([[0, 0], [ori_size[0] - 1, 0], [0, ori_size[1] - 1],
                           [ori_size[0] - 1, ori_size[1] - 1]]).float().to(offset.device).view(1, 4, 2).repeat(batch_size, 1, 1)
...
class DLT(nn.Module):
    ...
    def forward(self, src_pt, dst_pt, method='Axb'):
        ...
        if method == 'Ax0':
            ...
            H = torch.cat((torch.matmul(A_inv, B).view(-1, 8), src_pt.new_ones((self.batch_size, 1))), 1).view(self.batch_size, 3, 3)
            return H
...
def WarpImages(input_map, H, start, patch_size):
    ...
    new_batch_img = bilinear_interpote(
        input_map.permute(0, 2, 3, 1).view(bs, -1, imgc).contiguous(), offset_batch_xy[:, :, 0], offset_batch_xy[:, :, 1], patch_size, bs,
        imgh, imgw, imgc)
...
def CropPatchFromFull(patch_size, full_img, start, rescale=False):
    ...
    if rescale:
        new_batch_img = bilinear_interpote(
            full_img.permute(0, 2, 3, 1).view(bs, -1, c).contiguous(), batch_x, batch_y, patch_size, bs, h, w, c)
    else:
...
def make_gif(img1, img2, exp_name, name):
    ...
    with imageio.get_writer(f'gif_results/{exp_name}/{name}.gif', mode='I', duration=0.5, loop=0) as writer:
