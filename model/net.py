...
class DMHomo(nn.Module):
    ...
    def forward(self, data_batch, step=0):
        ...
        mask_fusion = (mask_fusion -
                       torch.min(mask_fusion)) / torch.max(mask_fusion)
...
def gen_N_basis(n, h, w, qr=True, scale=True):
    ...
    if n > 12:
        ...
        for _ in range(num_batches):
            batch_rand_flow = generate_random_homography_matrices(
                batch_size, h, w)
            _flow_ls.append(batch_rand_flow)

        if remaining > 0:
            remaining_rand_flow = generate_random_homography_matrices(
                remaining, h, w)
...
def print_unfrozen_modules(model):
    print("Unfrozen modules:")
    for name, module in model.named_modules():
        # 如果模块有参数，且至少有一个参数是requires_grad=True，则说明该模块是unfrozen的
...
class CamFlow(nn.Module):
    ...
    def __init__(self,
        ...
        if not os.path.exists(basis_path):
            if params.num_basis != 8:
                ...
                self.basis = gen_8_basis(
                    self.params.crop_size[0],
                    self.params.crop_size[1]).unsqueeze(0).reshape(
                        1, params.num_basis, -1)
                print(basis_path)
                torch.save(self.basis, basis_path)
        ...
        # freeze decoder fine-tune encoder
        if hasattr(params, '_freeze_decoder') and params._freeze_decoder:
            # unfreeze only the feature extractor, freeze other parts
            print('unfreezing only the feature extractor')
            for param in self.parameters():
                param.requires_grad = False

            for param in self.fea_extra.parameters():
                param.requires_grad = True

        # freeze encoder and fine-tune decoder
        if hasattr(params, '_freeze_feature_extractor'
                   ) and params._freeze_feature_extractor:
            # freeze only the feature extractor, keep other parts trainable
            print('freezing only the feature extractor')
            for param in self.parameters():
                param.requires_grad = True
        ...
        # freeze mask module
        if hasattr(params,
                   '_freeze_mask_generator') and params._freeze_mask_generator:
            print('freezing mask generator')
            for param in self.mask_generator.parameters():
                param.requires_grad = False

        if hasattr(params, '_unfreeze_blocks_token_noise_and_bothHeads'):
            if params._unfreeze_blocks_token_noise_and_bothHeads:
                self._unfreeze_blocks_token_noise_and_bothHeads()
    ...
    def _unfreeze_blocks_token_noise(self):
        print(
            'only unfreeze the parameters of blocks_token_noise in swinTransformer'
        )
        for param in self.parameters():
            param.requires_grad = False

        for param in self.h_net.blocks_token_noise.parameters():
            param.requires_grad = True

    def _unfreeze_blocks_token_noise_and_heads(self):
        print(
            'only unfreeze the parameters of blocks_token_noise in swinTransformer'
        )
        for param in self.parameters():
        ...
        for param in self.h_net.head1_noise.parameters():
            param.requires_grad = True

        for param in self.h_net.head2_noise.parameters():
            param.requires_grad = True

    def _unfreeze_blocks_token_noise_and_bothHeads(self):
        print(
            'unfreeze the parameters of blocks_token_noise and physical/noise heads in swinTransformer'
        )
        for param in self.parameters():
            param.requires_grad = False
    ...
    def normalize_fea(self, fea):
        ...
        _fea = torch.log(fea + 1e-8)
        _fea = torch.clamp(_fea, torch.zeros_like(_fea), torch.max(_fea))
        _fea = _fea / torch.max(_fea) * 255
    ...
    def forward(self, data_batch, step=0):
        ...
        if step % 2000 == 0 and self.training:
            ...
            mask_b_warp_vis = mask_b_warp
            # mask_b_warp_vis = self.normalize_fea(mask_b_warp_vis)

            log_var_f = -torch.log(mask_f + 1e-8)
            log_var_b = -torch.log(mask_b + 1e-8)

            nll_weight_f = math.sqrt(2) * torch.exp(-0.5 * log_var_f)
            nll_weight_b = math.sqrt(2) * torch.exp(-0.5 * log_var_b)

            print(f"[Step {step}] Mask Statistics:")
            print(
                f"  mask_f: mean={torch.mean(mask_f).item():.4f}, std={torch.std(mask_f).item():.4f}, min={torch.min(mask_f).item():.4f}, max={torch.max(mask_f).item():.4f}"
            )
            print(
                f"  mask_b: mean={torch.mean(mask_b).item():.4f}, std={torch.std(mask_b).item():.4f}, min={torch.min(mask_b).item():.4f}, max={torch.max(mask_b).item():.4f}"
            )
            print(
                f"  log_var_f: mean={torch.mean(log_var_f).item():.4f}, std={torch.std(log_var_f).item():.4f}, min={torch.min(log_var_f).item():.4f}, max={torch.max(log_var_f).item():.4f}"
            )
            print(
                f"  log_var_b: mean={torch.mean(log_var_b).item():.4f}, std={torch.std(log_var_b).item():.4f}, min={torch.min(log_var_b).item():.4f}, max={torch.max(log_var_b).item():.4f}"
            )
            print(
                f"  nll_weight_f: mean={torch.mean(nll_weight_f).item():.4f}, std={torch.std(nll_weight_f).item():.4f}, min={torch.min(nll_weight_f).item():.4f}, max={torch.max(nll_weight_f).item():.4f}"
            )
            print(
                f"  nll_weight_b: mean={torch.mean(nll_weight_b).item():.4f}, std={torch.std(nll_weight_b).item():.4f}, min={torch.min(nll_weight_b).item():.4f}, max={torch.max(nll_weight_b).item():.4f}"
            )
            ...
            buf_img_src = torch.cat([
                img1_patch,
                (img1_patch * (mask_f_warp)),
                img1_patch,
                (img1_patch * (mask_f_warp)),
            ], -1)
            buf_fea_src = torch.cat([
                img1_patch_fea,
                (img1_patch_fea * (mask_b_warp)),
                img1_patch_fea,
                (img1_patch_fea * (mask_b_warp)),
            ], -1)
            buf_src = torch.cat([buf_mask_src, buf_img_src, buf_fea_src], -2)
            ...
            buf_img_tar = torch.cat([
                img2_patch,
                (img2_patch * (mask_f_warp)),
                img2_patch_remap,
                (img2_patch_remap * (mask_f_warp)),
            ], -1)
            buf_fea_tar = torch.cat([
                img2_patch_fea,
                (img2_patch_fea * (mask_b_warp)),
                img2_patch_fea_remap,
                (img2_patch_fea_remap * (mask_b_warp)),
            ], -1)
            buf_tar = torch.cat([buf_mask_tar, buf_img_tar, buf_fea_tar], -2)
        ...
        return {
            'img1_patch': img1_patch.contiguous(),
            'img2_patch': img2_patch.contiguous(),
            'img1_patch_warp': img1_patch_remap.contiguous(),
            'img2_patch_warp': img2_patch_remap.contiguous(),
            'img1_full': img1_full.contiguous(),
            'img2_full': img2_full.contiguous(),
            'img1_rgb_full': img1_rgb_full.contiguous(),
            'img2_rgb_full': img2_rgb_full.contiguous(),
