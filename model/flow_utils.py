...
def get_corresponding_map(data):
    ...
    invalid = torch.cat([
        x_ceil_out | y_ceil_out, x_ceil_out | y_floor_out,
        x_floor_out | y_ceil_out, x_floor_out | y_floor_out
    ],
                        dim=1)
    ...
    indices = torch.cat([
        x_ceil + y_ceil * W, x_ceil + y_floor * W, x_floor + y_ceil * W,
        x_floor + y_floor * W
    ], 1).long()  # BxN   (N=4*H*W)
...
def get_occu_mask_bidirection(flow12, flow21, scale=0.01, bias=0.5):
    ...
    mag = (flow12 * flow12).sum(
        1, keepdim=True) + (flow21_warped * flow21_warped).sum(1, keepdim=True)
...
def euclidean(t):
    # 计算通道维度上的 L2 距离，shape: [B, H, W]
    return torch.norm(t, p=2, dim=1)

def flow_error_avg(pred_flow, gt_flow):
    B, _, H, W = gt_flow.shape
    _, _, h, w = pred_flow.shape
    assert (H == h) and (W == w), "Shape mismatch: {} vs {}".format(pred_flow.shape, gt_flow.shape)

    # 计算欧氏距离，shape: [B, H, W]
    diff = euclidean(pred_flow - gt_flow)

    # 计算每个样本的平均 EPE，shape: [B]
    avg_diff = diff.view(B, -1).mean(dim=1)

    # PCK 指标（每个样本的平均）
    PCK_1 = (diff <= 1.0).float().view(B, -1).mean(dim=1)
    PCK_5 = (diff <= 5.0).float().view(B, -1).mean(dim=1)

    return avg_diff, PCK_1, PCK_5
