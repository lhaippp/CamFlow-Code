...
class SwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        if param.num_basis == 8:
            self.physical_basis = self.basis[:, :8]
            self.noise_basis = self.basis[:, 8:]
        else:
            self.physical_basis = self.basis[:, :12]
            self.noise_basis = self.basis[:, 12:]
        ...
        # Generalized basis logic
        self.num_physical_basis = 8 if self.num_basis == 8 else 12
        self.num_noise_basis = 0 if self.num_basis == 8 else self.num_basis - 12

        self.query_token = nn.Parameter(
            torch.zeros(1, self.num_physical_basis, self.num_features))
        self.query_token_noise = nn.Parameter(
            torch.zeros(1, self.num_noise_basis, self.num_features))
        ...
        self.blocks_token_only = nn.ModuleList([
            LayerScale_Block_CA(dim=self.num_features,
                                ...
                                Mlp_block=Mlp,
                                basis_num=self.num_physical_basis)
            for i in blks_list
        ])

        self.blocks_token_noise = nn.ModuleList([
            LayerScale_Block_CA(dim=self.num_features,
                                out_dim=self.num_features,
                                num_heads=param.num_heads[i],
                                mlp_ratio=self.mlp_ratio,
                                ...
                                Attention_block=Class_Attention,
                                Mlp_block=Mlp,
                                basis_num=self.num_noise_basis)
            for i in blks_list
        ])
        ...
        self.head2 = nn.Linear(self.num_features, 1)  # for each basis

        self.norm_noise = norm_layer(self.num_features)
        self.avgpool_noise = nn.AdaptiveAvgPool1d(1)

        self.head1_noise = nn.Linear(self.num_features, self.num_features)
        self.head2_noise = nn.Linear(self.num_features,
                                     1)  # for each noise basis

        self.activate = self.activation()

        trunc_normal_(self.query_token, std=.02)
        trunc_normal_(self.query_token_noise, std=.02)
    ...
    def forward(self, x):
        ...
        query_token_noise = self.query_token_noise.repeat(bs, 1, 1)
        ...
        for l, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if self.num_noise_basis > 0:
                # noise basis tokenizer
                query_token_noise = self.blocks_token_noise[
                    self.num_layers - l - 1](query_token_noise, x)
                query_token_noise = self.norm_noise(query_token_noise)  # B L C
                # regression
                h_noise = self.activate(self.head1_noise(query_token_noise))
                h_noise = self.head2_noise(h_noise)
                # concat physical and noise basis
                h = torch.cat((h, h_noise), dim=1)
...
class Class_Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        q = self.q(x[:, :self.basis_num]).reshape(B, self.basis_num,
                                                  self.num_heads,
        ...
        x_cls = (attn @ v).transpose(1, 2).reshape(B, self.basis_num, C)
